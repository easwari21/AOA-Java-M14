...
1.Read the list of coin denominations and the target amount to make.

2. Create a dp[] array of size amount + 1, where dp[i] represents the minimum number of coins needed to make amount i.

3.For each amount i from 1 to amount, check every coin value,If the coin value ≤ i, update dp[i] = min(dp[i], dp[i - coin] + 1).

4.After filling the array, if dp[amount] > amount, it means the target amount cannot be formed — return -1.

5.Print dp[amount], the minimum number of coins needed to make the given amount. 
